...
earnings_by_user_type = pd.pivot_table(df, values="Action Earnings", index=pd.Grouper(freq='YE', key="action_date"), columns=["login_status"], aggfunc='sum', fill_value=0)
...
earnings_by_user_type.index = pd.to_datetime(earnings_by_user_type.index).strftime('%Y')
...
earnings_by_user_type_graph.set_xlabel("Year")
earnings_by_user_type_graph.set_ylabel("Total Earnings")
earnings_by_user_type_graph.set_title('Yearly earnings from Ticketmaster by user type')
...
earnings_by_user_type_figure.savefig('ticketmaster-by-user-type-year.png')